...
(function () {
  ...
  function injectCSS() {
    ...
    s.textContent = [
      ...
      /* Banner sticky */
      '#vlxt-mission-banner{position:fixed;bottom:20px;right:20px;z-index:99980;',
      'background:linear-gradient(135deg,#0d1b2a,#0d1117);',
      'border:1px solid rgba(0,240,255,.35);border-radius:99px;',
      'padding:0 12px 0 12px;height:38px;display:inline-flex;align-items:center;gap:8px;',
      'box-shadow:0 4px 20px rgba(0,114,255,.35);cursor:pointer;',
      'transition:all .15s ease;font-family:\'Inter\',system-ui,sans-serif;',
      'max-width:260px;box-sizing:border-box;animation:nvBanIn .3s ease-out;}',
      '@keyframes nvBanIn{from{opacity:0;transform:translateY(14px)}to{opacity:1;transform:none}}',
      '#vlxt-mission-banner:hover{border-color:#00f0ff;transform:translateY(-2px);box-shadow:0 6px 24px rgba(0,240,255,.4);}',
      '.nvb-icon{font-size:15px;line-height:1;display:inline-flex;align-items:center;justify-content:center;flex-shrink:0;transform:translateY(-1px);}',
      '.nvb-text{flex:1;min-width:0;display:inline-flex;align-items:center;transform:translateY(-1px);}',
      '.nvb-title{font-size:13px;font-weight:700;color:#e6edf3;line-height:1;white-space:nowrap;',
      'overflow:hidden;text-overflow:ellipsis;letter-spacing:.01em;}',
      '.nvb-close{width:18px;height:18px;border-radius:50%;background:rgba(255,255,255,.1);',
      'color:#8b949e;border:none;cursor:pointer;font-size:11px;line-height:1;',
      'flex-shrink:0;font-family:inherit;padding:0;display:inline-flex;',
      'align-items:center;justify-content:center;transition:all .15s ease;margin-left:2px;}',
      '.nvb-close:hover{background:rgba(255,255,255,.2);color:#fff;}',
      ...
      '.nv-btn{width:100%;}',
      '#vlxt-mission-banner{bottom:12px;right:12px;left:12px;max-width:none;justify-content:space-between;}}',
