...
- Ngày: 19/08/2026 (vòng 3, sau khi Codex xác minh vòng 2 test pass nhưng còn 2 blocker cuối — CHƯA chấp nhận merge)
- Người thực hiện: Claude
- Nhánh: `perf/cache-integrity-audit-20260819` — CẢ HAI repo, cùng nhánh với vòng 1/2, KHÔNG merge/push main, KHÔNG deploy.
- Publish: `git push` trực tiếp vẫn bị chặn bởi git-proxy của sandbox (hạ tầng) — dùng lại GitHub web-upload như vòng 1/2 (xem commit hash thật ở báo cáo gửi thầy, luôn ưu tiên xem `git log`/GitHub trực tiếp thay vì hash chép lại ở đây vì dễ trôi thông tin).
- Kết quả vòng 3 (2 blocker Codex nêu + fetch lại `origin/main`):
  1. **Quiz publish atomic thật sự**: tên file `data/quizzes/quiz-*.json` đổi từ hash-chỉ-theo-khoá (vòng 2, không atomic — có thể ghi đè file đang được index CŨ tham chiếu) sang CONTENT-ADDRESSED (`fileNameForEntry` trong `scripts/quiz-publish.mjs` — hash theo cả khoá bài học lẫn nội dung dòng câu hỏi, `quiz-[0-9a-f]{20}.json`). Nội dung không đổi → tên không đổi (bỏ qua ghi lại); nội dung đổi → tên MỚI hoàn toàn, không đụng file cũ. `quiz-index.json` giờ cutover qua ghi-file-tạm (`.quiz-index.json.tmp-*` trong cùng thư mục `data/`) rồi `rename()` — atomic trên POSIX. Thứ tự bắt buộc: ghi hết file mới → rename index → mới dọn file cũ. Thêm 3 test fault-injection trong `scripts/test-quiz-publish.mjs` (mock lỗi khi ghi file thứ 2, mock lỗi khi ghi file tạm của index, mock lỗi khi `rename()`) — cả 3 đều xác nhận `quiz-index.json` cũ và mọi file nó tham chiếu giữ nguyên byte-for-byte; file mới dư (nếu có) chấp nhận được, sẽ tự dọn ở lần chạy thành công sau. Tổng `scripts/test-quiz-publish.mjs`: **10/10 pass**.
  2. **CORS**: rà toàn bộ `doPost()` trong `apps-script-CAPNHAT.txt` — xác nhận KHÔNG có action chỉ-đọc/không-mutation nào dùng được để test an toàn. Đã thêm đề xuất action `pingadmin` (CHƯA deploy, CHƯA wire vào `doPost` — chỉ nằm dưới dạng comment hướng dẫn ở đầu `apps-script-CAPNHAT.txt` repo Admin) — chỉ xác thực `adminKey` rồi trả `{ok:true}`, không đụng Sheets. Thêm mục "Hướng dẫn test CORS Admin" trong `PROJECT_STATE.md` (các bước cụ thể bằng trình duyệt thật, static server cục bộ, DevTools). CORS vẫn là điều kiện BẮT BUỘC chặn merge Admin cho tới khi thầy tự deploy `pingadmin` và xác nhận đọc được JSON thật qua POST — trợ lý AI KHÔNG tự deploy Apps Script, KHÔNG thử sửa/xoá dữ liệu production.
  3. Rebase nhánh student lên `origin/main` mới nhất (có thêm commit data tự động `6cde61c`) trước khi publish vòng này — không đụng `data/` (thuộc workflow tự động), không force-push.
- Kiểm tra đã chạy sau khi sửa: `node --check` cho `scripts/quiz-publish.mjs`/`scripts/test-quiz-publish.mjs`/`scripts/sync-public-data.mjs`; `node scripts/test-quiz-merge.mjs` (6/6); `node scripts/test-quiz-publish.mjs` (10/10, gồm 3 test fault-injection mới). Admin không có thay đổi code trong vòng 3 (chỉ thêm đề xuất `pingadmin` dạng comment trong `apps-script-CAPNHAT.txt`, không phải file thực thi) nên không cần chạy lại `test-postAdminWrite.mjs` (không đổi) — vẫn xác nhận `node --check` qua được. CHƯA test trên trình duyệt thật/dữ liệu production thật.
- Bước tiếp theo: thầy tự deploy `pingadmin` (nếu đồng ý) và test CORS theo hướng dẫn; Codex/thầy review vòng 3; nếu đạt thì mới merge. Cập nhật mục này sau mỗi thay đổi đáng kể, không thêm một sổ bàn giao cạnh tranh.
